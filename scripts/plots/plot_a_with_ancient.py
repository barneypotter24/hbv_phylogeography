...
from cmcrameri import cm  # color maps
...
def determine_location(k):
    ...
    elif ("location.set" in k.traits.keys()) and (
        "location.set.prob" in k.traits.keys()
    ):
...
def plot_tip_labels(tre, ax):
    ...
    tre.addText(
        ax,
        position=text_pos_func,
        target=target_func_tips,
        text=text_func_tips,
        size=10,
        color="black",
    )
...
def time_delta_plot(tre, ax):
    ...
    for t in tips:
        time_to_ancestor_tips[t.name] = findTimeDelta(
            t.parent.absoluteTime, t.absoluteTime
        )
...
def add_tmrca_text(log, ax):
    ...
    ax.text(
        ax.get_xlim()[0] + 0.02,
        ax.get_ylim()[1] * 0.6,
        f"TMRCA: {(mean_tmrca)}\n\nHPD95%: {tmrca_hpd[0]} to {tmrca_hpd[1]}",
        fontsize=15,
    )
    ax.text(
        ax.get_xlim()[0] + 0.02,
        ax.get_ylim()[1] * 0.5,
        f"Clock rate: {mean_clock}\n\nHPD95%: {clock_hpd[0]} to {clock_hpd[1]}",
        fontsize=15,
    )
...
def add_static_map(tre, gjs, colors, ax):
    ...
    colors1 = cm.tokyo(np.linspace(0.0, 1, 400))[::-1]
    colors2 = cm.lipari(np.linspace(0.0, 1, 51))
    colorsforcmap = np.vstack((colors1, colors2))
    mycmap = mpl.colors.LinearSegmentedColormap.from_list("my_colormap", colorsforcmap)

    travel_lineages = []
    for k in tre.Objects:
        try:
            if k.parent != tre.root and determine_location(k) != determine_location(
                k.parent
            ):
    ...
    xDates = [
        n for n in range(1, math.ceil(tre.root.traits["height"]) + 1)
    ]  ## create timeline
    heights = [
        k.absoluteTime for k in travel_lineages
    ]  ## get absolute times of each branch in the tree
    height_normalization = create_normalization(
        [-2500, 2014], 0.0, 1.0
    )  ## create a normalization based on timeline, where earliest day is 0.0 and latest is 1.0

    cmap = mycmap  ## colour map
    ...
    for k in travel_lineages[
        ::-1
    ]:  ## iterate through lineages which have switched location
        ...
        normalized_height = height_normalization(
            k.absoluteTime
        )  ## normalize heights of lineages
        normalized_parent_height = height_normalization(k.parent.absoluteTime)

        distance = math.sqrt(
            math.pow(oriX - desX, 2) + math.pow(oriY - desY, 2)
        )  ## find travelling distance

        adjust_d = -1 * (
            distance * (1 + transition_counts[locB][locA] / 5) / 5
        )  ## position Bezier curve control point according to an arbitrary function

        n = Bezier_control(
            (oriX, oriY), (desX, desY), adjust_d
        )  ## control point perpendicular to midway between point A and B at a distance adjust_d

        curve = Bezier(
            [(oriX, oriY), n, (desX, desY)], 0.0, 1.0, num=30
        ...
        for i in range(
            ...
            ax.plot(
                ...
                lw=1 + 4 * (1 - frac),
                color=cmap(
                    normalized_parent_height
                    + (normalized_height - normalized_parent_height) * (1 - frac)
                ),
                zorder=int((1 / normalized_height) * 10000),
            )  ## curve tapers and changes colour
    ...
    country_region = {
        row["country"]: row["continent"] for index, row in country_region_df.iterrows()
    }
    ...
    for i, loc in enumerate(locations):  ## iterate over locations
        ...
        ax.add_collection(
            PatchCollection(
                polygons[loc], facecolor=regionColor, edgecolor="black", lw=1, zorder=-1
            )
        )  ## polygon colour pale

        lon, lat = region_coords[region]  ## population centroid coordinates

        size = [determine_location(k) for k in tre.Objects].count(
            loc
        )  ## circle size proportional to branches in location
        size = 50 + size
        ax.scatter(
            lon,
            lat,
            size,
            facecolor=desaturate(regionColor, 1.0),
            edgecolor="k",
            lw=2,
            zorder=200000,
        )  ## plot circle, edge coloured inversely from main colour
    ...
    ax2 = ax.get_figure().add_axes(
        [0.38, 0.18, colorbarHeight, colorbarWidth]
    )  ## add dummy axes

    mpl.colorbar.ColorbarBase(
        ax2,
        cmap=mycmap,
        norm=mpl.colors.Normalize(-2500, 2014.0),
        orientation="horizontal",
    )
    ax2.xaxis.set_major_locator(
        mpl.ticker.LinearLocator(numticks=10)
    )  ## add colour bar to axes
    ...
    ax2.set_xlabel(
        "Transition Time", color="k", size=colorbarTextSize
    )  ## colour bar label is "date"
    ax2.tick_params(
        labelcolor="k", size=18, labelsize=colorbarTickLabelSize, labelrotation=45
...
def plot_BEAST(tre, gjs, log, o_file):
    ...
    fig, (ax0, ax1, ax2, ax3) = plt.subplots(
        1, 4, figsize=(21, 15), gridspec_kw={"width_ratios": [1, 1, 1, r]}, dpi=300
    )
    ...
    #### first  plot ####
    #####################
    tre.plotTree(ax=ax0, x_attr=x_attr, colour_function=c_func, branchWidth=w_func)
    ...
    #### second  plot ####
    ######################
    tre.plotTree(ax=ax1, x_attr=x_attr, colour_function=c_func, branchWidth=w_func)
    mrsd = max([x.absoluteTime for x in tre.getExternal()])
    ...
    #### third  plot ####
    #####################
    tre.plotTree(ax=ax2, x_attr=x_attr, colour_function=c_func, branchWidth=w_func)
    mrsd = max([x.absoluteTime for x in tre.getExternal()])
    ...
    #### fourth  plot ####
    ######################
    tre.plotTree(ax=ax3, x_attr=x_attr, colour_function=c_func, branchWidth=w_func)
    mrsd = max([x.absoluteTime for x in tre.getExternal()])
    ...
    ax0.yaxis.tick_left()
    ax0.tick_params(
        labelleft=False, axis="y", width=0, length=0
    )  # put tick labels at the left
    ax1.tick_params(
        labelleft=False, axis="y", width=0, length=0
    ...
    lax = plt.axes([0.13, 0.5, 0.05, 0.05])
    add_legend(cmap, lax)
    map_location = (0.13, 0.16)  # location of map inset within larger figure
    map_ar = ((5 / 7) * 0.62, 0.3)  # aspect ratio of map inset
    map_scale = 1.25  # scale of map inset
    inside = plt.axes(
        [map_location[0], map_location[1], map_ar[0] * map_scale, map_ar[1] * map_scale]
    )
...
if __name__ == "__main__":
    ...
    log_file = log
    # plot tree
    mcc_tree = bt.loadNexus(
        f"{folder}/{filestem}.mcc.tre",
        variableDate=True,
        tip_regex="\/([\-0-9]+)",
        date_fmt="%Y",
    )
