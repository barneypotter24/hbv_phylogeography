...
def determine_loc_probability(k,someLoc):
    if someLoc not in k.traits['location.set']:
        return 0.0
    else:
        i = k.traits['location.set'].index(someLoc)
        return float(k.traits['location.set.prob'][i])
...
def add_static_map(tre, gjs, colors, ax):
    ...
    mpl.colorbar.ColorbarBase(ax2, cmap=mpl.cm.get_cmap('cividis_r'),norm=mpl.colors.Normalize(-2500,2014.),orientation='horizontal')
...
def plot_BEAST(tre,gjs,log,o_file):
    ...
    target_func=lambda k: (k.branchType=='node') and determine_location(k) == 'Africa' and determine_loc_probability(k,'Americas')>=0.01## only target nodes
    text_func=lambda k: '%.2f'%(determine_loc_probability(k,'Europe')) ## what text is plotted
    # text_func=lambda k: ", ".join(k.traits['location.set.prob'])
    kwargs={'va':'top','ha':'right','size':20} ## kwargs for text
    tre.addText(ax3,position=pos_func,target=target_func,text=text_func,**kwargs) ## text
...
if __name__ == '__main__':
    ...
    plot_file = f"{folder}/{output}_annotated.pdf"
